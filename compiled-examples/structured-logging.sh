...
export WORKFLOW_NAME="structured-logging-demo"
...
if [ "$_should_run" = "false" ]; then
  ...
  _log job "test" action "checkout" event "→" "Starting"
  ...
  if [ "$NIXACTIONS_LOG_FORMAT" = "simple" ]; then
    ...
    /nix/store/2krqy1vnpzq5hg9fbxyrpkfhgb9znvrn-checkout/bin/checkout
    _action_exit_code=$?
  else
    # Structured/JSON format - wrap each line
    /nix/store/2krqy1vnpzq5hg9fbxyrpkfhgb9znvrn-checkout/bin/checkout 2>&1 | _log_line "test" "checkout"
  ...
  if [ $_action_exit_code -ne 0 ]; then
    ACTION_FAILED=true
    _log job "test" action "checkout" duration "${_action_duration_s}s" exit_code $_action_exit_code event "✗" "Failed"
    # Don't exit immediately - let conditions handle flow
  else
    _log job "test" action "checkout" duration "${_action_duration_s}s" exit_code $_action_exit_code event "✓" "Completed"
  fi
fi
...
if [ "$_should_run" = "false" ]; then
  echo "⊘ Skipping install-deps (condition: $ACTION_CONDITION)"
else
  _log job "test" action "install-deps" event "→" "Starting"
  ...
  if [ "$NIXACTIONS_LOG_FORMAT" = "simple" ]; then
    # Simple format - pass through unchanged
    /nix/store/m9sknx1wm251l49srkpjgi35wg5myij4-install-deps/bin/install-deps
    _action_exit_code=$?
  else
    # Structured/JSON format - wrap each line
    /nix/store/m9sknx1wm251l49srkpjgi35wg5myij4-install-deps/bin/install-deps 2>&1 | _log_line "test" "install-deps"
    _action_exit_code=${PIPESTATUS[0]}
  fi
  set -e
  ...
  if [ $_action_exit_code -ne 0 ]; then
    ACTION_FAILED=true
    _log job "test" action "install-deps" duration "${_action_duration_s}s" exit_code $_action_exit_code event "✗" "Failed"
    # Don't exit immediately - let conditions handle flow
  else
    _log job "test" action "install-deps" duration "${_action_duration_s}s" exit_code $_action_exit_code event "✓" "Completed"
  fi
fi
...
if [ "$_should_run" = "false" ]; then
  ...
  if [ "$NIXACTIONS_LOG_FORMAT" = "simple" ]; then
    # Simple format - pass through unchanged
    /nix/store/021zbpvims9hahjn0lfsxz9s13q1n2rj-run-tests/bin/run-tests
    _action_exit_code=$?
  else
    # Structured/JSON format - wrap each line
    /nix/store/021zbpvims9hahjn0lfsxz9s13q1n2rj-run-tests/bin/run-tests 2>&1 | _log_line "test" "run-tests"
    _action_exit_code=${PIPESTATUS[0]}
  fi
  set -e
