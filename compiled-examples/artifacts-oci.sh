...
export WORKFLOW_NAME="artifacts-simple-oci"
...
if [ "$_should_run" = "false" ]; then
  ...
  if [ "$NIXACTIONS_LOG_FORMAT" = "simple" ]; then
    ...
    /nix/store/vg6djpdrsax8wz5jssvjva7bgx551vn7-build/bin/build
    _action_exit_code=$?
  else
    # Structured/JSON format - wrap each line
    /nix/store/vg6djpdrsax8wz5jssvjva7bgx551vn7-build/bin/build 2>&1 | _log_line "build" "build"
...
if [ "$_should_run" = "false" ]; then
  ...
  if [ "$NIXACTIONS_LOG_FORMAT" = "simple" ]; then
    # Simple format - pass through unchanged
    /nix/store/pfbs6ccripni4pmq8ll9x1c34am6zxpv-test/bin/test
    _action_exit_code=$?
  else
    # Structured/JSON format - wrap each line
    /nix/store/pfbs6ccripni4pmq8ll9x1c34am6zxpv-test/bin/test 2>&1 | _log_line "test" "test"
    _action_exit_code=${PIPESTATUS[0]}
  fi
  set -e
