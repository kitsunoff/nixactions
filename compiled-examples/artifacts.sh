...
export WORKFLOW_ID WORKFLOW_NAME="artifacts-simple"
...
run_action "build" "build" "/nix/store/6aj0lvfzqyp05m5ii498nf8nhivdng7v-build/bin/build" 'success()' 'date +%s%N 2>/dev/null || echo "0"'
...
save_local_artifact "myapp" "myapp" "build"
...
restore_local_artifact "dist" "test"

_log_job "test" artifact "dist" event "✓" "Restored"

restore_local_artifact "myapp" "test"
...
run_action "test" "test" "/nix/store/dfsi7aq678iahd9kkdn3lqhn66l3fpa6-test/bin/test" 'success()' 'date +%s%N 2>/dev/null || echo "0"'
