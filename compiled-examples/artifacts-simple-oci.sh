...
run_action "build" "build" "/nix/store/smlr0xn0d4i7qjgvz4l4k28psbmmjy6j-build/bin/build" '\''success()'\'' '\''date +%s%N 2>/dev/null || date +%s'\''
...
run_action "test" "test" "/nix/store/f7n2nj35lcgfp9wnsc77ykk5mg31br3g-test/bin/test" '\''success()'\'' '\''date +%s%N 2>/dev/null || date +%s'\''
