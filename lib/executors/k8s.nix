...
makeConfigurable {
  ...
  defaultConfig = {
    ...
    namespace = "default";
    registry = {
      url = null;           # e.g., "ghcr.io/myorg"
      usernameEnv = null;   # e.g., "REGISTRY_USER"
      passwordEnv = null;   # e.g., "REGISTRY_PASSWORD"
    };
    ...
    # Kubeconfig
    kubeconfigEnv = null;   # e.g., "KUBECONFIG" - path to kubeconfig file
    contextEnv = null;      # e.g., "KUBE_CONTEXT" - context name to use
    ...
    # Timeouts
    podReadyTimeout = 300;  # 5 minutes
  };
  
  # Function that creates executor from config
  make = { 
    namespace ? "default",
    registry ? { url = null; usernameEnv = null; passwordEnv = null; },
    mode ? "shared",
    copyRepo ? true,
    name ? null,
    extraPackages ? [],
    ...
    assert lib.assertMsg (registry.url != null) "K8s executor requires registry.url";
    assert lib.assertMsg (registry.usernameEnv != null) "K8s executor requires registry.usernameEnv";
    assert lib.assertMsg (registry.passwordEnv != null) "K8s executor requires registry.passwordEnv";
    ...
    let
      ...
      # Generate node selector args
      nodeSelectorArgs = lib.concatStringsSep "" (
        lib.mapAttrsToList (k: v: " --overrides='{\"spec\":{\"nodeSelector\":{\"${k}\":\"${v}\"}}}'") nodeSelector
      );
